...
API.prototype = {
	...
	// check an email for a valid subscription
	check: function( options, callback ){
		// fallbacks
		options = options || {};
		callback = callback || function(){};
		// prerequisites
		if( !options.email ) return callback('no_data');
		// variables
		var self = this,
			product, data;
		...
		var actions = [
			...
			function( done ){
				...
				// continue if we already have the data
				if( product ) return done();
				// request product info
				self.read("product", function(err, result){
					// error control
					if( err ) return done(err);
					self.set({ product: result });
					// save reference
					product = result;
					// continue...
					done();
			...
			// check email against app
			function( done ){
				//
				self.read("v1/product/"+ product.id +"/email/"+ options.email, function(err, result){
					// error control
					if( err ) return done(err);
					// save reference
					data = result || false;
